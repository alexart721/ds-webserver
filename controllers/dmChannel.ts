...
const getAllDmChannelForUser = async (req: ExtendedRequest, res: Response): Promise<void> => {
  try {
    const getAllDmChannel = await Users.findById(req.user._id);
...
const createNewDmChannel = async (req: ExtendedRequest, res: Response): Promise<void> => {
  try {
    const newDmChannel = await DmChannels.create(req.body.dmChannel);
    await Users.findOneAndUpdate({ _id: req.user._id }, { $push: newDmChannel._id });
...
const addNewMessage = async (req: ExtendedRequest, res: Response): Promise<void> => {
...
// DELETE a dm channel
const deleteDmChannel = async (req: ExtendedRequest, res: Response): Promise<void> => {
  try {
